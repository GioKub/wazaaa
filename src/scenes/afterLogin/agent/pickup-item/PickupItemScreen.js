...
const PickupItemScreen = ({ navigation }) => {
  ...
  //     console.log(json, "<---jsssooon")
  //     // setParcels(json)
  // }).then(({data}) => {
      
  // }).catch((error) => {
  //     console.error(error, "<---something went wrong with automatic login")
  // }).finally(() => {
  ...
  useEffect(()=>{
    fs.readAsStringAsync(sessionPath).then((raw) => {
      const json = JSON.parse(raw);
      setParcels(json.parcels, "<---start")
      console.log("parcels taken from data")
    })
  ...
  useEffect(() => {
    if (parcels.length > 0) {
      const toWrite = JSON.stringify({parcels});
  ...
  const addReceiver = () => {
    book({ email: emailToBookWith }).then((data) => {});
    validate(sender)
      .then(() => {
  ...
  useEffect(() => {
    ...
    if (UK2GE && GE2UK) {
      setSender({ ...sender, country_code: "UK" });
    }
      ...
      book({ email: emailToBookWith })
        .then((data) => {
          ...
          if (data.data.error == true) {
            ...
            let i = 0;
            console.log(idsToSkip, "<---parcel ids to skip");
            ...
            if (i >= data.data.book.items.length) {
              ...
              setGotHereFromBooking(false);
              alert(
                "you have already requested all the items for this booking or all the times that are left have already been booked"
              );
            }
  ...
  return (
    <>
      <PreventGoingBack navigation={navigation} shouldAlert={true} />
      <ScrollView style={container}>
        <View style={{ borderWidth: 0, flex: 5.1 }}>
          ...
          {/* {console.log(sender.country_code)} */}
